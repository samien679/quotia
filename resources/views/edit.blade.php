<x-app-layout>
    ...
    <x-slot name="info2">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Kokonaiskate: ') }}<strong>{{ $quote->sales_margin_value }}</strong>
            {{ __('€') }} {{ $quote->sales_margin_percentage }}{{ __('%') }}
    </x-slot>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    ...
                    <div>
                        <table
                            class="w-full text-right bg-green-200 border-collapse border-green-600 table-auto shadow-md">
                            <thead>
                                <tr class="font-bold text-gray-800">
                                    <th class="py-1 pr-2 bg-green-500 border border-green-600 text-right">Tuotekoodi
                                    </th>
                                    ...
                                    <th class="py-1 pr-2 bg-green-500 border border-green-600 text-right">Ostohinta á
                                    </th>
                                    <th class="py-1 pr-2 bg-green-500 border border-green-600 text-right">Myyntihinta á
                                    </th>
                                    <th class="py-1 pr-2 bg-green-500 border border-green-600 text-right">Yhteensä</th>
                                    <th class="py-1 pr-2 bg-green-500 border border-green-600 text-right">Kate / rivi
                                    </th>
                            ...
                            <tbody>
                                ...
                                @foreach ($quoteItems as $quoteItem)
                                    ...
                                    <tr class="pt-3">
                                        <td class="py-1 pr-2 border border-green-600">
                                            {{ $quoteItem->product_number }}</td>
                                        <td class="py-1 pr-2 border border-green-600">
                                            {{ $quoteItem->name1 }}{{ $quoteItem->name2 }}</td>
                                        <td class="py-1 pr-2 border border-green-600">{{ $quoteItem->qty }}
                                            {{ $quoteItem->unit }}</td>
                                        <td class="py-1 pr-2 border border-green-600">
                                            {{ $quoteItem->purchase_price_decimals }}{{ __(' €') }}</td>
                                        <td class="py-1 pr-2 border border-green-600">
                                            {{ $quoteItem->quote_price_decimals }} {{ __(' €') }}</td>
                                        <td class="py-1 pr-2 border border-green-600">
                                            {{ $quoteItem->item_value }}{{ __(' €') }}</td>
                                        <td class="py-1 pr-2 border border-green-600">
                                            {{ $quoteItem->sales_margin_percentage }}{{ __('%  ') }}{{ $quoteItem->sales_margin_row }}{{ __('€') }}
                                        </td>
                                        <td class="py-1 pr-2 border border-green-600">
                                            {{ $quoteItem->vat }}{{ __(' %') }}</td>
                                        <td class="text-center py-1 pr-2  border border-green-600">
                                            <div class="flex flex-row justify-around">
                                                ...
                                                <form method="POST"
                                                    action="{{ route('quote_items.destroy', $quoteItem->id) }}">
                                                    @csrf
                    ...
                    <form method="POST" action="{{ route('quote_items.store') }}">
                        ...
                        <div class="flex flex-wrap justify-start border rounded-lg shadow-sm p-3 mt-4">
                            ...
                            <div>
                                ...
                                <x-input id="product_number" class="flex mt-1" type="text" name="product_number"
                                    :value="old('product_number')" autofocus />
                            </div>
                            ...
                            <div>
                                ...
                                <x-input id="qty" class="flex mt-1" type="number" step="0.01" name="qty"
                            ...
                            <div>
                                ...
                                <x-input id="unit" class="flex mt-1" type="text" name="unit" :value="old('unit')"
                                    required />
                            </div>
                            ...
                            <div>
                                ...
                                <x-input id="purchase_price" class="flex mt-1" type="number" step="0.01"
                            ...
                            <div>
                                ...
                                <x-input id="quote_price" class="flex mt-1" type="number" step="0.01" name="quote_price"
                                    :value="old('quote_price')" required />
                            </div>
                            ...
                            <div>
                                ...
                                <x-input id="supplier" class="flex mt-1" type="text" name="supplier"
                                    :value="old('supplier')" />
                            </div>
